...
export async function POST(request: Request) {
  try {
    const { email, name, trackName } = await request.json()
    ...
    await resend.emails.send({
      ...
      html: `
        <div style="font-family: -apple-system, sans-serif; max-width: 560px; margin: 0 auto; padding: 40px 20px;">
          <h1 style="color: #fff; font-size: 28px;">You're Enrolled! 📚</h1>
          <p style="color: #9ca3af; font-size: 16px; line-height: 1.6;">
            Hey ${name}, you've successfully enrolled in <strong style="color: #60a5fa;">${trackName}</strong>.
          </p>
          <p style="color: #9ca3af; font-size: 16px; line-height: 1.6;">
            Start completing tasks to work toward your certificate. Good luck!
          </p>
